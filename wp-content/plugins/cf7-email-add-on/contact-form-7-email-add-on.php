...
function cf7_email_add_on_activate() {	
	...
	if( ! has_action( 'wpcf7_init' ) ) {
		...
		wp_die( __( 'Please activate Contact Form 7.', 'cf7-email-addon' ), 'Plugin dependency check',
			array(
				'back_link' => true
			)
		);
 ...
 */
function cf7ea_init() {
	load_plugin_textdomain( 'cf7-email-addon', false, basename( dirname( __FILE__ ) ) . '/languages' );
	// Load core class
